...
class PPO(nn.Module):
    def __init__(self, device, state_dim, action_dim, args):
        ...
        self.critic = Critic(args.layer_num, state_dim, 1,\
                             self.args.hidden_dim, torch.relu, self.args.last_activation)
    ...
    def get_action(self,state):
        ...
        #print(action)
        #log_prob = torch.log_prob(action).sum(-1, keepdim = True)
        #log_prob = torch.nn.functional.log_softmax(action, dim=-1)
