...
class Agent(nn.Module):
    def __init__(self,algorithm, writer, device, state_dim, action_dim, args, demonstrations_location_args): 
        ...
        #self.expert_states = torch.tensor(np.concatenate([np.load(f, allow_pickle=True) for _ in range(file_size)])).float()
        self.expert_states = torch.tensor(np.concatenate([np.load("./data/expert_state_500.npy") for _ in range(file_size)])).float()
        f = open(demonstrations_location_args.expert_action_location,'rb')
        self.expert_actions = torch.tensor(np.concatenate([np.load("./data/expert_action_500.npy") for _ in range(file_size)]))
        f = open(demonstrations_location_args.expert_next_state_location,'rb')
        self.expert_next_states = torch.tensor(np.concatenate([np.load("./data/expert_next_state_500.npy") for _ in range(file_size)])).float()
        f = open(demonstrations_location_args.expert_done_location,'rb')
        self.expert_dones = torch.tensor(np.concatenate([np.load("./data/expert_done_500.npy") for _ in range(file_size)])).float().unsqueeze(-1)
        f.close()
    ...
    def train(self, discriminator, discriminator_batch_size, state_rms, n_epi, batch_size = 1):
